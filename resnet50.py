...
class Resnet50(nn.Module):
    def __init__(self):
        ...
        self.classify = nn.Sequential(
            nn.Linear(in_features=204800, out_features=5, bias=True)
            )
    def forward(self,x):
        #####layer1#####
        x = self.conv0(x)
        temp = x
        temp = self.downsample_1(temp)
        x = self.conv1_0(x)
        x = x + temp
        x = F.relu(x)
        x = self.conv1_1(x)
...
def execute(net,string):
    ...
    np.save('ResNet50_'+string+'_' +'testingdata',test_accuracy)    
    np.save('ResNet50_'+string+'_' +'trainingdata',train_accuracy) 
    
    """    
    plt.title('Resnet50' ,fontsize=14)
    plt.xlabel('epoch')
    plt.ylabel('accuracy')
...
"""
def save(net,string):
  torch.save(net.state_dict(),string + 'Resnet50.pth')

def load(net,string):
  netload= net
  netload.load_state_dict(torch.load(string + 'Resnet50.pth'))
...
if args.cuda:
    ...
    resnet50.to(device)
optimizer = optim.SGD(resnet50.parameters(), lr=args.lr)
#execute(net = resnet50,string='original')
load(net = resnet50,string = 'original')
#plt.show()
#confusionmatrix(data=test_loader,net=resnet50,string='original')
...
pretrain_ResNet50.avgpool = nn.AdaptiveAvgPool2d(output_size=(1,1))
pretrain_ResNet50.fc = nn.Linear(2048 , 5)
if args.cuda:
    device = torch.device('cuda')
    pretrain_ResNet50.to(device)
optimizer = optim.SGD(pretrain_ResNet50.parameters(), lr = args.lr, weight_decay = 5e-4)
#execute(net = pretrain_ResNet50,string = 'pretrain')
#load(net = pretrain_ResNet50,string = 'pretrain')
#plt.show()
#confusionmatrix(data = test_loader,net = pretrain_ResNet50,string = 'pretrain')
